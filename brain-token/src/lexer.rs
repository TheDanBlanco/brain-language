...
use crate::{stream::TokenStream, Brain, attributes::Attribute, token::Token};
...
impl<T> Lexer<T> where T:  Clone + std::fmt::Debug + PartialEq {
    pub fn new() -> Self {
        Lexer {
    ...
    fn scan_token(&mut self) -> Result<(), Box<dyn std::error::Error>> {        
        for attribute in &self.attributes {
            match attribute {
                Attribute::Literal(literal) => {
                    ...
                    self.tokens.push(Token::new(span.clone(), literal.token.clone(), Some(next.to_string())));
                ...
                Attribute::Regex(regex_attribute) => {
                    ...
                    self.tokens.push(Token::new(span.clone(), regex_attribute.token.clone(), Some(capture.to_string())));
        ...
        return Err(
            Error::new(
                ...
                format!("No remaining tokens matched for : {}", &self.input[self.current..])
            )
        )
    }
